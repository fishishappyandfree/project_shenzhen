...
class Config(object):
    ...
    seq_height = 312                       # 把一维序列整成二维 (seq_height,seq_width)=(32,64)=2048
    seq_width = 512

    num_classes = 3                       # Softmax 的类别数
    fc_size = 256                          # 全连接层fc1大小


    filter_height = [25, 25, 25]             # 每个filter对应的height
    filter_width  = [25, 25, 25]             # 每个filter对应的width
    ...
    pool_ksize = [[1, 5, 5, 1],                  # 每一层池化层的 池化窗口size
                  [1, 2, 2, 1],
                  [1, 2, 2, 1]]

    pool_strides = [[1, 5, 5, 1],                # 每一层池化层的 池化窗口移动的步长strides
                    [1, 2, 2, 1],
                    [1, 2, 2, 1]]
