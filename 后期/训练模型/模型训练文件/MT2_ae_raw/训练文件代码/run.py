...
def plot_save(array, fig_path=None, xlabel='epochs', ylabel='loss value', dpi=400):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(fig_path, (ylabel + '.tif')), dpi=dpi)
    # try:
...
def load_data(file_path):
    ...
    train_data = np.concatenate([normal_train_data, inner_train_data, outer_train_data], axis=0)
    train_label = np.concatenate([normal_train_label, inner_train_label, outer_train_label], axis=0)
    test_data = np.concatenate([normal_test_data, inner_test_data, outer_test_data], axis=0)
    test_label = np.concatenate([normal_test_label, inner_test_label, outer_test_label], axis=0)
...
if __name__ == '__main__':
    ...
    # 导入数据，取原始数据的 80% 作为训练集， 20% 作为测试集
    # 定义保存的文件夹
    file_path = 'MT2_ae_dataset.h5'
    # 加载数据
    train_data, train_label, test_data, test_label = load_data(file_path)
    num_train_label = np.arange(len(train_label))
    num_test_label = np.arange(len(test_label))
    np.random.shuffle(num_train_label)
    np.random.shuffle(num_test_label)
    train_data = train_data[num_train_label]
    train_label = train_label[num_train_label]
    test_data = test_data[num_test_label]
    test_label = test_label[num_test_label]
    end=time.time()
    print(end-start)

    # dataset_path = os.path.join(Config.PATH, 'MT2_ae_dataset_mean.mat')
    # dataset_path = 'MT2_ae_dataset_mean.mat'
