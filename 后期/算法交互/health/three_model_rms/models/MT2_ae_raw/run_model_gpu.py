...
from models.MT2_ae_raw.CNN_model import CNN
from models.MT2_ae_raw.Config import Config
...
class Model(object):
    ...
    def run_train(self, samples_train, labels_train, checkpoint_path_folder):
        '''
        samples_train: 是一个二维数组,(500,159744)
        ...
        self.config_train.learning_rate = 0.005  
        self.config_train.dropout_keep_prob_fc = 1 
        self.config_train.batch_size = 4
    ...
    def run_test(self, x_test, checkpoint_path_folder):
        ...
        x_test = numpy.array(x_test).reshape(-1,159744)
        ...
        with tf.Session(config=self.gpu_config) as sess_test:
            ...
            return index, show_pro_fault_pred_class, fault_probility_lable[0]

            
if __name__ == "__main__":
    data = numpy.array(range(159744)).reshape(1,159744)
    model = Model()
    model.run_test(data)
