...
signalx = numpy.array(signal)  # transfer to numpy.ndarray
L = len(signal)


hx = fftpack.hilbert(signalx)
signal_baoluopu = numpy.sqrt(signalx**2+hx**2)


# PL = abs(numpy.fft.fft(signal_baoluopu / L))[: int(L / 2)]
# PL = abs(numpy.fft.fft(signal_baoluopu))[: int(L / 2)]


# PL[0] = 0
f = numpy.fft.fftfreq(L, 1)
plt.plot(signal[320000:360000])
plt.plot(signal_baoluopu[320000:360000], 'r')
plt.title('envelope')
